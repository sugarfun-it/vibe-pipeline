...
10. **orchestrator spawn 的 sub-agent 永遠帶 `--dangerously-skip-permissions`** — code orchestrator 派 codex sub-agent 時,sub-agent 內部 Bash 在 `defaultMode: auto` 下會被 permission_denials 擋(且 sub-agent 會幻覺成功訊息)。adapter 永遠帶 flag,不條件式偵測 provider。
