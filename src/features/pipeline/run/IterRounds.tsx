...
export function IterRounds({ rounds }: { rounds: IterRound[] }) {
  return (
    <div className="tdrw-iter-rounds">
      {rounds.map((r) => {
        // 一輪要寫了 endedAt 才算完成;之前是進行中,不顯佔位 verdict(否則誤顯 PARTIAL)。
        // doer 階段(還沒 executorSummary)= 執行中;critic 階段(已有 summary)= 審核中。
        const pending = !r.endedAt;
        const runningExec = pending && !r.executorSummary;
        const pendingLabel = runningExec ? "執行中" : "審核中";
          ...
          <div key={r.n} className="tdrw-iter-round">
            <div className="tdrw-iter-round-head">
              ...
              <span
                ...
                title={pending ? pendingLabel : r.criticVerdict}
                aria-label={pending ? `本輪${pendingLabel},尚未產生審核結果` : `審核結果 ${verdictLabel}(原值 ${r.criticVerdict})`}
              >
                {verdictLabel}
              </span>
            ...
            <div className="tdrw-iter-round-block">
              ...
              {r.criticFeedback ? (
                ...
                <div className="tdrw-text tdrw-feedback-empty">
                  {pending ? (runningExec ? "（執行中…）" : "（審核中…）") : n === "PASS" ? "（通過，無補充意見）" : "（無 feedback）"}
                </div>
              )}
            </div>
          </div>
