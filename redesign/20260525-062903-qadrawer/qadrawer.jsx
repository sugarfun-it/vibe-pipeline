...
function App() {
  ...
  const [pipelineName, setPipelineName] = useState("auth-fallback-pipeline");
  const [showCloseConfirmDemo, setShowCloseConfirmDemo] = useState(false);

  // Mirror theme onto <html>
  useEffect(() => {
  ...
  // 模擬 close-confirm:把 composer 內容塞個非空字串
  // 為了讓 user 看到 close-confirm 視圖,直接以 demo control 切 pendingClose state
  // QADrawer 內部 requestClose 透過 composerTextRef trigger,demo 改用 wrapper 暴露 setPendingClose
  // 為了不改動 verbatim 元件,demo 用 ref hack:塞 textRef + 自動呼 requestClose
  const composerInjectRef = useRef(null);
  useEffect(() => {
    if (!showCloseConfirmDemo) return;
    // 給 demo:塞文字到 textarea,模擬 unsent content
    const id = requestAnimationFrame(() => {
      const ta = document.getElementById("qadr-composer-textarea");
      if (ta) {
        ta.value = "（demo: 假裝這裡有未送出的文字）";
        ta.dispatchEvent(new Event("input", { bubbles: true }));
        // 用 ESC 等同的方式去 trigger requestClose
  ...
  return (
    <div className="host-shell">
      <div className="host-fake-board">
        <div className="host-fake-topbar">
          <span className="mono">vibe-pipeline</span>
          <span>/</span>
          <span className="mono">{pipelineName}</span>
          <span style={{ flex: 1 }} />
          <span style={{ fontSize: 11 }}>scenario: <code>{scenario}</code></span>
        </div>
      </div>

      <QADrawer
        key={scenario /* 切 scenario 重 mount,避免 SpecReview 的 edited state 卡舊值 */}
      ...
      <aside className="demo-panel" role="region" aria-label="QADrawer demo controls">
        <div className="demo-section">
          <h4>Scenario</h4>
          <div className="demo-row">
            <select
              value={scenario}
              onChange={(e) => { setShowCloseConfirmDemo(false); setScenario(e.target.value); }}
            >
              <option value="bootstrap">1. bootstrap (no draft, busy)</option>
              <option value="welcome">2. welcome (empty turns + starters)</option>
              <option value="dialogSingle">3. dialog + single quick reply</option>
              <option value="dialogMulti">4. dialog + InlineMultiSelect</option>
              <option value="thinking">5. waitingForAI (助理思考中)</option>
              <option value="ready">6. spec 5/5 + ready-bar (chat view)</option>
              <option value="reviewSimple">7. review (no split)</option>
              <option value="reviewSplit">8. review + split-proposal (3)</option>
            </select>
          </div>
        </div>
        ...
        <div className="demo-section">
          ...
          <div className="demo-row">
            <button
              className={"demo-btn" + (theme === "dark" ? " is-on" : "")}
              onClick={() => setTheme("dark")}
            >dark</button>
            <button
              className={"demo-btn" + (theme === "light" ? " is-on" : "")}
        ...
        <div className="demo-section">
          <h4>Notes</h4>
          <div style={{ fontSize: 10, color: "var(--fg-mute)", lineHeight: 1.5 }}>
            scenario 7-8 自動進 review 視圖(complete=true)。在 review 內按「繼續討論」可回 chat 視圖,
            然後若 spec 仍 5/5 會看到 ready-bar 出現(viewOverride sticky)。
          </div>
        </div>
      </aside>
    </div>
