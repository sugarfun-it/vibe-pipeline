// HTTP request dispatch:CORS + static serve + /api/* route table。
// 從 server/index.ts split 出來,server/serve.ts 的 Bun.serve fetch handler 用 handle()。
...
export function logAccess(method: string, pathname: string, status: number, startedAt: number): void {
...
export function withCors(response: Response, origin: string | null, requestHeaders?: string | null): Response {
