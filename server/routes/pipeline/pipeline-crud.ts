...
import { getResolvedDefaults } from '../../lib/domain/projectConfig';
...
export async function listPipelines(hash: string): Promise<Response> {
  return withProject(hash, async (project) => ok(await pipelineStore.listPipelines(project.path)));
}
...
export async function createPipeline(hash: string, req: Request): Promise<Response> {
  ...
  const id = (body.id as string) || pipelineStore.generatePipelineId(name);
  ...
  if (typeof body.autoMerge === "boolean") {
    ...
    const resolved = await getResolvedDefaults(project.path);
  ...
  await pipelineStore.writePipeline(project.path, id, data, { source: "api-create-pipeline", sourceDetail: `POST /pipelines name=${name}` });
  return ok(data);
  }));
}
...
export async function savePipeline(hash: string, id: string, req: Request): Promise<Response> {
  return withProject(hash, async (project) => withJsonBody(req, async (body) => {
  const existing = (await pipelineStore.readPipeline(project.path, id)) as {
    state?: string;
  } | null;
  ...
  const data = { ...body, id };
  await pipelineStore.writePipeline(project.path, id, data, {
