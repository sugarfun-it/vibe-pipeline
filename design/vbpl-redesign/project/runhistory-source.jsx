/* ============================================================================
 * runhistory-source.jsx — verbatim source extracted from
 * src/features/pipeline/RunHistory.tsx (via uploads/runhistory.jsx).
 *
 * Components: RunHistory, RunCard, StdoutBlock, CopyableBlock
 * Helpers exposed at top scope (consumed by the redesign harness):
 *   fmtDuration, fmtNum, fmtTime, formatFailureReason,
 *   localizeResult, renderResult, computeTicketDiff, useCopiedFeedback,
 *   STATE_LABELS, TICKET_STATUS_LABELS, STDOUT_PREVIEW_LINES, visuallyHidden,
 *   ChevronIcon
 *
 * The redesign harness (runhistory-redesign.jsx) consumes these and wraps
 * them in its own demo shell with a scenario picker. DOM/className identical
 * to source — visual changes come from runhistory-redesign.css only.
 * ========================================================================= */

const { useEffect: useEffectRH, useId: useIdRH, useMemo: useMemoRH, useState: useStateRH, useRef: useRefRH } = React;
...
function useCopiedFeedback() {
  const [copied, setCopied] = useStateRH(false);
  const tref = useRefRH(null);
  const flash = () => {
    setCopied(true);
...
function RunHistory({
  ...
  const [runs, setRuns] = useStateRH(initialRuns !== undefined ? initialRuns : null);
...
function RunCard({ run, projectHash, pipelineId, fetchDetail }) {
  const [open, setOpen] = useStateRH(!!run._initialOpen);
  const [detail, setDetail] = useStateRH(run._initialDetail ?? null);
  const [detailLoading, setDetailLoading] = useStateRH(!!run._initialDetailLoading);
  const [detailFailed, setDetailFailed] = useStateRH(!!run._initialDetailFailed);
  const headId = useIdRH();
  const detailId = useIdRH();
...
function StdoutBlock({ text }) {
  const [expanded, setExpanded] = useStateRH(false);
  const lines = useMemoRH(() => text.split(/\r?\n/), [text]);
